 ...
 * SIZE constraint, for lengths outside the extension root (vlm/asn1c PR #548).
 *
 * X.691:2021 30.4: outside the extension root the value is encoded "as if
 ...
 * pin the X.691:2021 30.5.4 translation rules: FROM(" ".."@") (ub = 2^b,
 ...
 *
 * The expected byte streams are golden interoperability vectors. Their bit
 * layouts are derived from X.691:2021 30.4 and 30.5 and are asserted together
 * with the exact encoded and consumed bit counts.
 */
#undef	NDEBUG
#include <stdio.h>
#include <stdlib.h>
