...
{\ttfamily -D~\emph{<directory>}} & {\small Place the generated files into the specified destination directory. The default is the current directory.}\\
...
{\ttfamily -Wdebug-lexer} & {\small Enable the lexer debugging during the ASN.1 lexing stage.}\\
...
\midrule
{\ttfamily -fall-defs-global} & {\small When generating code for types embedded inside another type, expose
the inner type's descriptor and helper functions globally (non-static) instead of hiding them as file-local.}\\
...
{\ttfamily -fcomplex-threshold=\emph{<value>}} & {\small Threshold value, in number of members, beyond which the compiler
switches a structure's members to indirect (pointer) storage instead of storing them inline. Must be between 1 and 500; the default is 4.}\\
...
{\ttfamily -fprefer-import-source} & {\small Require an explicit name listing in the IMPORTS group (xp\_members) for a symbol to be
considered as imported from that group, instead of falling back to scanning the whole body of the FROM module. Fixes ambiguous same-name imports.}\\
...
{\ttfamily -no-gen-JER} & {\small Do not generate the JSON Encoding Rules (JER, X.697) support code. By default, JER support code is generated.}\\
...
{\ttfamily -gen-autotools} & {\small Generate an example top-level \texttt{configure.ac} and \texttt{Makefile.am} for the generated code.}\\
{\ttfamily -gen-BER}, {\ttfamily -gen-XER}, {\ttfamily -gen-JER}, {\ttfamily -gen-CBOR}, {\ttfamily -gen-OER}, {\ttfamily -gen-UPER}, {\ttfamily -gen-APER}, {\ttfamily -gen-print}, {\ttfamily -gen-random-fill}, {\ttfamily -gen-example}, {\ttfamily -no-gen-autotools} & {\small Inverses of the switches above. Every encoding rule and every support artifact except the autotools example is generated by default, so each of these restates the default and has no effect on its own. They matter in combination: switches are applied in command line order, so \texttt{-no-gen-XER -gen-XER} generates the XER support code, and \texttt{-gen-autotools -no-gen-autotools} does not generate the autotools example. This is useful where the switches are accumulated by a build system rather than typed by hand. Note also that \texttt{-fno-constraints} is rejected while any of \texttt{-gen-OER}, \texttt{-gen-UPER} or \texttt{-gen-APER} is in effect, which by default is always.}\\
...
{\ttfamily -flink-skeletons} & {\small Symlink the runtime skeleton support files into the destination directory instead of copying them. Not available on Windows, where files are always copied.}\\
{\ttfamily -flist-deps} & {\small List PDU dependencies (requires \texttt{-pdu} option, no code generated).}\\ \\
...
Applications that need to skip a complete raw CBOR item can call
\code{cbor\_skip\_item()}.  Code that runs as part of a decoder and has an
\code{asn\_codec\_ctx\_t} should call \code{cbor\_skip\_item\_with\_ctx()}
instead, so recursive arrays, maps, and tags are bounded by the same stack
limit as normal ASN.1 decoding:
...
\section{\label{sec:Unknown-Extension-Decoding}Unknown extension decoding}

\textbf{Upgrade warning.} Decoding behavior for unknown extensions has changed
from ``fail'' to ``skip/relay''. Evaluate the impact on your application before
upgrading. To restore the previous strict behavior, use the build definition
shown below.

The forward-compatible default applies when an older schema decodes a value
produced using a newer version of an extensible ASN.1 type:

\begin{itemize}
\item An unknown UPER or OER \code{CHOICE} extension alternative is consumed
and skipped. The decoder returns \code{RC\_OK}, with no local alternative
selected.
\item An unknown UPER \code{ENUMERATED} extension addition is represented in
the reserved \code{LONG\_MAX - extension\_index} range. Re-encoding that value
with UPER relays the original extension index.
\end{itemize}

The change does not make truncated or structurally malformed encodings valid.
It does mean that an unknown, structurally valid extension no longer produces
\code{RC\_FAIL}. Applications that used that failure as an implicit input-
validation gate, or whose state machines treat every decode failure as a
protocol error, should review their behavior before upgrading.
...
Apply the definition consistently to all generated/runtime skeleton objects and
perform a clean rebuild. Defining the macro only in application code does not
change decoder objects in an already-built runtime library. Enabling strict
rejection forfeits forward compatibility for these extension additions.
