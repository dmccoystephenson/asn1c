...
:   Inverses of the switches above. Every encoding rule and every support
    artifact except the autotools example is generated by default, so each of
    these restates the default and has no effect on its own. They matter in
    combination: switches are applied in command-line order, so
    **-no-gen-XER -gen-XER** generates the XER support code, and
    **-gen-autotools -no-gen-autotools** does not generate the autotools
    example. This is useful where the switches are accumulated by a build
    system rather than typed by hand. Note also that
    **-fno-constraints** is rejected while any of **-gen-OER**,
    **-gen-UPER** or **-gen-APER** is in effect, which by default is always.
